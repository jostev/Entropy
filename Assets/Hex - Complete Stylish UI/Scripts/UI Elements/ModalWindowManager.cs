...
namespace Michsky.UI.Hex
{
    ...
    public class ModalWindowManager : MonoBehaviour
    {
        ...
        // Settings
        public bool liftNavigationRestrictions = false;
        ...
        void InitModalWindow()
        {
            ...
            openStateLength = HexUIInternalTools.GetAnimatorClipLength(mwAnimator, "ModalWindow_In");
            closeStateLength = HexUIInternalTools.GetAnimatorClipLength(mwAnimator, "ModalWindow_Out");
        }
        ...
        {
            if (liftNavigationRestrictions || IsNavigationInFocus())
                return;

            if (cancelButton != null && !IsNavigationInFocus()) { ControllerManager.instance.SelectUIObject(cancelButton.gameObject); }
            else if (confirmButton != null && !IsNavigationInFocus()) { ControllerManager.instance.SelectUIObject(confirmButton.gameObject); }
        }

        bool IsNavigationInFocus()
        {
            if (cancelButton != null && EventSystem.current.currentSelectedGameObject == cancelButton.gameObject) { return true; }
            else if (confirmButton != null && EventSystem.current.currentSelectedGameObject == confirmButton.gameObject) { return true; }
            else { return false; }
        }
