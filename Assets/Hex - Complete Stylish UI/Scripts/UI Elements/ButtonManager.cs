...
namespace Michsky.UI.Hex
{
    ...
    public class ButtonManager : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, ISelectHandler, IDeselectHandler, ISubmitHandler
    {
        ...
        [HideInInspector] public ButtonDimmer dimmer;
#if UNITY_EDITOR
        ...
        void OnEnable()
        {
            ...
            if (EventSystem.current != null && EventSystem.current.currentSelectedGameObject == gameObject) { OnSelect(null); }
        ...
        public void UpdateUI()
        {
            ...
            if (enableText)
            {
                ...
                if (highlightTextObj != null)
                {
                    ...
                    if (!Application.isPlaying && tooltipObject != null) { tooltipObject.SetActive(false); }
                    else if (tooltipObject != null) { tooltipObject.SetActive(true); }
                }
        ...
        {
            if (dimmer != null && dimmer.allowGamepad) { dimmer.DimButtons(this); }
            if (!isInteractable) { return; }
            if (useSounds) { UIManagerAudio.instance.audioSource.PlayOneShot(UIManagerAudio.instance.UIManagerAsset.hoverSound); }
        ...
        {
            if (dimmer != null && dimmer.allowGamepad) { dimmer.LitButtons(); }
            if (!isInteractable) { return; }
