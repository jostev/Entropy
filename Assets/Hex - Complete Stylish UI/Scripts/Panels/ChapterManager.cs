...
namespace Michsky.UI.Hex
{
    public class ChapterManager : MonoBehaviour
    {
        ...
        {
            if (!backgroundStretch || currentBackgroundRect == null || !gameObject.activeInHierarchy)
                return;
        ...
        public void InitializeChapters()
        {
            ...
            for (int i = 0; i < chapters.Count; ++i)
            {
                ...
                // Title
                if (!useLocalization || string.IsNullOrEmpty(chapters[i].titleKey)) { item.titleObject.text = chapters[i].title; }
                else
                {
                ...
                // Description
                if (!useLocalization || string.IsNullOrEmpty(chapters[i].descriptionKey)) { item.descriptionObject.text = chapters[i].description; }
                else
                {
